...
def store_canonical_apk(release: AppRelease) -> None:
    """Store uploaded APK as releases/livetv-{platform}-v{version}-b{build}.apk."""
    if not release.apk_file:
        return

    canonical = canonical_apk_basename(
        release.platform,
        release.version_name,
        release.build_number,
    )
    storage_name = f"releases/{canonical}"
    ...
    release.save(update_fields=["apk_file", "updated_at"])
